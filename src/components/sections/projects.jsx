...
const Projects = () => {
  const loadProjectsMarkdownFiles = async () => {
    ...
    const markdownFiles = await Promise.all(
      filePaths.map(async filePath => {
        ...
        const { data : frontmatter, content: html } = matter(fileContent);
  ...
  useEffect(() => {
    const fetchProjectsData = async () => {
      const files = await loadProjectsMarkdownFiles();
  ...
  const GRID_LIMIT = 6;
  const projects = markdownFiles.filter(({ frontmatter }) => frontmatter);
  const firstSix = projects.slice(0, GRID_LIMIT);
  const projectsToShow = showMore ? projects : firstSix;

  const projectInner = infoNode => {
    const { frontmatter, html } = infoNode;
  ...
  return (
    <StyledProjectsSection>
      ...
      <Link className="inline-link archive-link" to="" ref={revealArchiveLink}>
        view the archive (Work In Progress)
      </Link>
